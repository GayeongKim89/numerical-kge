...
def load_data(dataset, bfs_level=3, relabel=False):
    if dataset in ['train20','new0','new1','train2','train4','train6','train8','train40','train60','train80','comp3','new_g','new_t3','new_t4','new_t2','new_t5','new_t1','new_50','new_t_50','new_t','fb','new','Sparse-YAGO15k-75', 'FB15k-237', 'Sparse-FB15k-237', "YAGO15k","Comp7"]:
...
class RGCNLinkDataset(object):
    ...
    def load(self):
        ...
        literal_path = os.path.join(self.dir, 'literals/numerical_literals.txt')

        #print(train_path)
        with open(entity_path, 'rb') as fw:
            entity_dict = pickle.load(fw)
        with open(relation_path, 'rb') as fw:
            relation_dict = pickle.load(fw)

        df = pd.read_csv(literal_path, header=None, sep='\t')
        df1 = pd.read_csv(train_path, header=None, sep='\t')

        M = df1.shape[0]
        X = np.zeros([M, 3], dtype=int)
        for i, row in df1.iterrows():
            X[i, 0] = entity_dict[row[0]]
            X[i, 1] = relation_dict[row[1]]
            X[i, 2] = entity_dict[row[2]]
        ...
        train_npy = os.path.join(self.dir, 'train.npy')
