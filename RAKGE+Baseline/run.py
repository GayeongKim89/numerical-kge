...
from model.lte_models import TransE, DistMult, ConvE, TransMS


from model.rakge_model import DistMult_Gate_att,  TransE_Gate_att, ConvE_Gate_att
# from model.our_model import DistMult_Gate_att,  TransE_Gate_att, ConvE_Gate_att
from model.literal import DistMultLiteral_gate, ComplExLiteral_gate, ConvELiteral_gate, TransELiteral_gate, KBLN
...
class Runner(object):
    def __init__(self, params):
        ...
        self.num_ent, self.train_data, self.valid_data, self.test_data, self.num_rels, self.entity_dict, self.relation_dict = self.data.num_nodes, self.data.train, self.data.valid, self.data.test, self.data.num_rels, self.data.entity_dict, self.data.relation_dict
        self.triplets = process({'data':self.data, 'train': self.train_data, 'valid': self.valid_data, 'test': self.test_data},
                                self.num_rels)
    ...
    def fit(self):
        ...
        self.logger.info(
            f"MRR: Tail {test_results['mrr']:.5}")  # , Head {test_results['right_mrr']:.5}, Avg {test_results['mrr']:.5}")
        self.logger.info(
            f"MR: Tail {test_results['mr']:.5}")  # , Head {test_results['right_mr']:.5}, Avg {test_results['mr']:.5}")
        self.logger.info(
            f"hits_left@1 = {test_results['hits@1']}")
        self.logger.info(
            f"hits_left@3 = {test_results['hits@3']}")
    ...
    def train(self):
        ...
        for step, (triplets, labels,pos_neg) in enumerate(train_iter):
            ...
            if self.p.gpu >= 0:
                triplets, labels, pos_neg = triplets.to("cuda"), labels.to("cuda"), pos_neg.to("cuda")
            #print(triplets)
            subj, rel, obj  = triplets[:, 0], triplets[:, 1], triplets[:, 2]

            if 'Literal' in self.p.name:
                pred, loss2 = self.model(self.g, subj, rel, labels)
            elif self.p.literal == True:
                if self.p.name == 'KBLN':
                    pred, loss2 = self.model(self.g, subj, rel, labels)
                else:
                    pred, loss2 = self.model(self.g, subj, rel, obj, labels, pos_neg)

            elif self.p.name[-3:] =='lte':
                pred, loss2 = self.model(self.g, subj, rel, labels)
            else:
                pred = self.model(self.g, subj, rel)  # [batch_size, num_ent]
            # else:
            #     pred, loss2 = self.model(self.g, subj, rel, labels)
            if self.p.name[-3:] =='lte':
                ...
                # print(loss)
                # print(loss2)
            if self.p.literal == True: #and tolerance >= 10:

                loss += self.p.scale*loss2
    ...
    def evaluate(self, split):
        ...
        def get_combined_results(left, right):
            ...
            count_right = float(right['count'])
            # results['mr'] = round(left['mr'] / count, 5)
            # results['mrr'] = round(left['mrr'] / count, 5)
            results['left_mr'] = round(left['mr'] / count_left, 5)
            results['left_mrr'] = round(left['mrr'] / count_left, 5)
            results['right_mr'] = round(right['mr'] / count_right, 5)
            results['right_mrr'] = round(right['mrr'] / count_right, 5)
            results['mr'] = round((left['mr'] + right['mr']) / (count_left+count_right), 5)
            results['mrr'] = round(
                (left['mrr'] + right['mrr']) / (count_left+count_right), 5)
            for k in [1, 3, 10]:
                results[f'left_hits@{k}'] = round(left[f'hits@{k}'] / count_left, 5)
                results[f'right_hits@{k}'] = round(
                    right[f'hits@{k}'] / count_right, 5)
                results[f'hits@{k}'] = round(
                    (left[f'hits@{k}']+right[f'hits@{k}']) / (count_left+count_right) , 5)
                # results[f'hits@{k}'] = round(
                #     (results[f'left_hits@{k}'] + results[f'right_hits@{k}']) / 2, 5)
                # results[f'hits@{k}'] = round(left[f'hits@{k}'] / count, 5)
            return results

        self.model.eval()
        left_result = self.predict(split, 'tail')
        # right_result = self.predict(split, 'head')
        res = get_combined_results(left_result, left_result)
        # res = get_combined_results(left_result, left_result)
    ...
    def get_model(self):
        ...
        # literal 추가
            ...
            if self.p.name == 'DistMult_Gate_att':
                model = DistMult_Gate_att(self.num_ent, self.num_rels, numerical_literals, params=self.p)

            elif self.p.name == 'ComplEx_Gate_att':
                model = ComplEx_Gate_att(self.num_ent, self.num_rels, numerical_literals, params=self.p)

            elif self.p.name == 'TransE_Gate_att':
                model = TransE_Gate_att(self.num_ent, self.num_rels, numerical_literals, params=self.p)

            elif self.p.name == 'ConvE_Gate_att':
                model = ConvE_Gate_att(self.num_ent, self.num_rels, numerical_literals, params=self.p)
            ...
            # LiteralE
            elif self.p.name == 'DistMultLiteral_gate':
                model = DistMultLiteral_gate(self.num_ent, self.num_rels, numerical_literals, params=self.p)

            elif self.p.name == 'ComplExLiteral_gate':
                model = ComplExLiteral_gate(self.num_ent, self.num_rels, numerical_literals, params=self.p)

            elif self.p.name == 'ConvELiteral_gate':
                model = ConvELiteral_gate(self.num_ent, self.num_rels, numerical_literals, params=self.p)
            ...
            elif self.p.score_func.lower() == 'transms':
                model = TransMS(self.num_ent, self.num_rels, params=self.p)
            else:
                raise NotImplementedError
...
if __name__ == '__main__':
    ...
    parser.add_argument('--neg', dest='neg', type=int, default=5,
                        help='num_neg')
    ...
    parser.add_argument('--scale',dest='scale',default=0.1, type=float)
    parser.add_argument('--tolerance', dest='tolerance', default=400, type=int)
